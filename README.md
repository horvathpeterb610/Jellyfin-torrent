...
On televisions. Android TV, Fire TV and Roku clients are native apps with no web dashboard, so nothing a server plugin does can appear in them — the browse page is reachable from those only through a browser on the TV or on a phone. LG webOS and Samsung Tizen clients are jellyfin-web in a webview, so the page loads there like any dashboard page. Either way it is built for a remote: arrow keys move between releases, OK sends the one in focus, up from the top row returns to the search box, down from the bottom row reaches the pager, and the TV mode button enlarges the whole page (everything is sized in em, so one font-size on the root scales the layout). GET /TorrentBrowser redirects to the browse page, because the dashboard URL is not something anyone will type on an on-screen keyboard.

One rule that came out of testing with keys rather than a mouse: never disable a button the user is focused on. Disabling blurs it, so focus falls to the document and a remote user has to navigate back from the top of the page after every send. The Send button stays enabled and guards on its own state instead.

Torznab cannot sort. The indexer answers newest first and there is no standard sort parameter, so the most seeded release can be sitting on any page. The browse page therefore keeps every release it has fetched for the current criteria in one buffer, sorts and filters that, and pages through it itself; the indexer's own paging only ever widens the buffer. Opening the page pulls three indexer pages in the background before the ordering is worth trusting, and the pager pulls more on demand when you walk past the end. "Page 1 of 3+" means three pages so far, with more to fetch; the + disappears once a short page proves the indexer is out. A failed fetch stops the buffer growing rather than retrying the same broken indexer on every page turn.
